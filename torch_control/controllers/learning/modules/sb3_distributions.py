...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution(
        ...
        :return:
        """
        action_std = th.ones_like(mean_actions) * log_std.exp().clamp(min=STD_MIN, max=STD_MAX)
...
class SquashedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def __init__(self, action_dim: int, epsilon: float = 1e-6, transform_logp: bool = True,):
        super().__init__(action_dim)
        # Avoid NaN (prevents division by zero or log of zero)
        self.epsilon = epsilon
        self.gaussian_actions: Optional[th.Tensor] = None
        self.transform_logp = transform_logp
    ...
    def log_prob(self, actions: th.Tensor, gaussian_actions: Optional[th.Tensor] = None) -> th.Tensor:
        ...
        if self.transform_logp:
            # this comes from the fact that tanh is bijective and differentiable
            log_prob -= th.sum(TanhBijector.log_prob_correction(gaussian_actions), dim=1)
...
class StateDependentNoiseDistribution(Distribution):
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        ...
        if self.bijector is not None and self.transform_logp:
            # Squash correction (from original SAC implementation)
...
class TanhBijector:
    ...
    @staticmethod
    def log_prob_correction(x: th.Tensor) -> th.Tensor:
        # We use a formula that is more numerically stable, see details in the following link
        # https://github.com/tensorflow/probability/blob/master/tensorflow_probability/python/bijectors/tanh.py#L69-L80
        # This formula is mathematically equivalent to `tf.log1p(-tf.square(tf.tanh(x)))`, 
        # however this code is more numerically stable.
        #  Derivation:
        #    log(1 - tanh(x)^2)
        #    = log(sech(x)^2)
        #    = 2 * log(sech(x))
...
class ChunkDiagGaussianDistribution(Distribution):
    """
    Distribution for action chunking with independent Gaussian distributions for each action in the chunk.
    
    :param action_dim: Dimension of the action space.
    :param chunk_size: Number of actions in each chunk.
    """
    ...
    @property
    def mean(self):
        return self.distribution.mean.reshape(-1, self.chunk_size, self.action_dim)
    
    @property
    def variance(self):
        return self.distribution.variance.reshape(-1, self.chunk_size, self.action_dim)

    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0) -> Tuple[nn.Module, nn.Parameter]:
        mean_actions = nn.Linear(latent_dim, self.action_dim * self.chunk_size)
        log_std = nn.Parameter(th.ones(self.action_dim * self.chunk_size) * log_std_init, requires_grad=True)
        return mean_actions, log_std
    ...
    def log_prob(self, actions: th.Tensor, action_dist: Distribution, reduction: str='none') -> th.Tensor:
    ...
    def entropy(self, action_dist: Distribution) -> Optional[th.Tensor]:
        raise NotImplementedError
        return sum_independent_dims(action_dist.entropy())
    
    def sample(self) -> th.Tensor:
        return self.distribution.rsample().reshape(-1, self.chunk_size, self.action_dim)
    ...
    def log_prob_from_params(self, *args, **kwargs) -> Tuple[th.Tensor]:
        raise NotImplementedError